...
def outlier_batch(request):
    ...
    return render(request, 'dal_facility_analysis/outlier_batch.html', {
        'factories': factories,
        'lines': lines,
        'processes': processes,
...
def outlier_batch_ajax(request):
    ...
    result_ = FactoryAnalysis.objects.mongo_aggregate([
        {'$project': {
            ...
            'isBetween': {
                '$and': [{
                    '$lte': [{'$dateFromString': {'dateString': '$product_date'}}, {'$dateFromString': {'dateString': end_dt}}]
                }, {
                    '$gte': [{'$dateFromString': {'dateString': '$product_date'}}, {'$dateFromString': {'dateString': start_dt}}]
                }]
            }
            
        }},
        {'$match': {'$and': [{'factory': factory}, {'line': line}, {'process': process}, {factor_name: {'$ne': ''}},  {'isBetween': True}]}}
    ])
    ...
    if df.size > 0:
        ...
        try:
            ...
            for _, rows in df.iterrows():
                result.append({'product_date': rows.product_date, 'result': rows[factor_name], 'outlier': rows['outlier']})
    ...
    return JsonResponse({'factory': factory, 'line': line, 'process': process, 'factor': factor_name, 'results': result, 'data_q': data_q})
...
def correlation(request):
    ...
    return render(request, 'dal_facility_analysis/correlation.html', {
        'factories': factories,
        'lines': lines,
        'processes': processes,
    })
